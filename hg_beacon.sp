...
public OnPluginStart()
{
	...
	cvarPluginEnable = CreateConVar("sm_beacontimer_enabled", "1", "Включить или выключить таймер-маячок", _, true, 0.0, true, 1.0);
	cvarTime = CreateConVar("sm_beacontimer_time", "3.0", "Время ожидания таймера-маячка", _, true, 5.0, true, 600.0);
	g_Cvar_BeaconRadius = CreateConVar("sm_beacontimer_radius", "375", "Установить радиус для маяка.", 0, true, 50.0, true, 1500.0);
	
	AutoExecConfig(true, "beacon", "sourcemod/hunger_games");
